namespace PseudoParcial.Windows
{
    public partial class Form1 : Form
    {
        ...
        private double CalcularVolumen(double radio, double altura) => (Math.PI * Math.Pow(radio, 2) * altura) / 3;

        private double CalcularArea(double radio, double generatriz) => Math.PI * radio * (radio + generatriz);

        private double CalcularGeneratriz(double radio, double altura) => Math.Sqrt(Math.Pow(radio, 2) + Math.Pow(altura, 2));
